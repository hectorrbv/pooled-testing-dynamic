...
class ItemSelectionEnv(gym.Env):
    def __init__(self, num_items=50, max_selection=5):
        ...
        # Observation space: clearance values + utility values + binary indicators for selection
    ...
    def _get_state(self):
        """Return the state: Clearance + Utility + Selection Status."""
        return np.concatenate([self.clearance_values, self.utility_values / 3.0, self.selected])
...
def select_items(model, clearance_values, utility_values):
    ...
    num_items = len(clearance_values)  # Ensure correct item count
...
clearance_values = np.random.uniform(0, 1, 50)  # Example clearance values (0 to 1)
