...
# %%
def load_RL_model_if_exists(model, draws, epoch, beta, beta_decay, lr, save_path='models_init_augr/'):
    model_filename = f"{save_path}dynamic_RL_model_N{N}_B{B}_s{draws}_e{epoch}_b{beta}_lr{lr}_bd{beta_decay}.pth"
...
class UtilityMaximizingRLModel(nn.Module):
    ...
    def _initialize_weights(self):
        ...
        # Set high nonzero_count bias for final layer to ensure sigmoid outputs ~1
...
def draw_actions(probabilities):
    return torch.bernoulli(probabilities)  # Draws 0 or 1 based on the probabilities
