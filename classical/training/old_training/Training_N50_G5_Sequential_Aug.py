...
# %%
def load_RL_model_if_exists(model, draws, epoch, beta, beta_decay, lr, save_path='new_models_G_Aug/'):
    model_filename = f"{save_path}dynamic_RL_model_N{N}_Seq_G{G}_B{B}_s{draws}_e{epoch}_b{beta}_lr{lr}_bd{beta_decay}_Aug.pth"
...
training_data = getAgentsFromTrainData(f'training_data_N{N}_Seq_G{G}_B{B}_G{G}.csv')[:num_draws]
eval_data = getAgentsFromTrainData(f'eval_data_N{N}_Seq_G{G}_B{B}_G{G}.csv')[:num_draws]
