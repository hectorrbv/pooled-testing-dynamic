...
Let $N=3$ with tests $t_1=\{0,1,2\}$ and $t_2=\{1,2\}$, both returning a count of
$1$. The augmented planner learns there is exactly one infected individual in the
whole group and exactly one among $\{1,2\}$; that single infection must lie in
$\{1,2\}$, so individual $0$ is \emph{healthy} with certainty --- and a
deduced-healthy individual is precisely what can later be harvested for welfare. A
binary planner sees both pools as positive and can conclude nothing about $0$:
$t_1{=}+$ is already implied by $t_2{=}+$. It is the exact count, not the binary
outcome, that frees individual $0$ --- and this gap is what overlapping designs
with intermediate counts turn into welfare.
\end{example}
