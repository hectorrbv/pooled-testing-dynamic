...
is small at low budget and grows with scale ($+0.6\%$ at $N{=}3$ to $+4.0\%$ at
$N{=}5$ and larger at $N{=}7$), matching the intuition that count outcomes pay
...
-\mathcal{U}^{D}$, growing from $+0.6\%$ at $N{=}3$ to $+4.0\%$ at $N{=}5$, with a
larger gap at $N{=}7$ (single-budget regimes show the smallest gap, as only one
...
\begin{table}[t]
\centering
\caption{Mean welfare over $200$ instances/config (corrected code). The last
column is the augmented benefit $\mathcal{U}^{D}_{A}-\mathcal{U}^{D}$ as a
percentage of $\mathcal{U}^{D}$.}
\label{tab:hier}
\small
...
7 & 3 & 7 & \multicolumn{6}{l}{\emph{(running; fill from results/hierarchy/hierarchy\_n7.csv)}} \\
\bottomrule
\end{tabular}
\end{table}
