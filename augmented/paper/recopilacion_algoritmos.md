...
- **Estado:** maduro.
- **Hallazgos:** tests dedicados 5/5 (`tests_state_reward_greedy.py`), incluido `test_beta_zero_matches_standard_greedy_eu`. Registrado en `phase3_findings.ipynb`: (1) en alta prevalencia p_vip=0.8 beta NO cambia nada porque P(r=0)=0.2^k colapsa y el término miope domina; (2) en prevalencia moderada p_vip=0.35 beta=1.0 desplaza el primer pool de tamaño ~2 a 5+ y mejora ligeramente la EU. **Recomendación:** beta es valioso solo en prevalencia moderada (p~0.2-0.4). Usado en `sprint3_experiments.py`. Inconsistencia menor: default `info_metric='entropy'` pero `run_vip_benchmark` usa 'confirmed'.
...
- **Estado:** maduro.
- **Hallazgos:** 14/14 tests (`tests_hybrid.py`): `test_hybrid_k0_matches_dp` (K=0 reproduce el DP), `test_hybrid_kB_matches_greedy` (K=B reproduce el greedy), `test_hybrid_monotonic` (EU monótona creciente en K), validación de `latent_state_aware_score` (alpha=1 == miope, alpha=0 == info-gain). Probado con selector MOSEK (`test_hybrid_with_mosek_pool_selector`). Usado en `large_trees_exploration.ipynb` para medir el gap de optimalidad. Por construcción su EU domina (≥) la del miope para K>0.
