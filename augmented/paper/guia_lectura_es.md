...
Esta guía acompaña el borrador `dynamic_augmented_pooled_testing.tex`. Explica,
...
subconjunto t responde solo si hay o no algún infectado. El preprint binario
dinámico de Lopez, Marmolejo-Cossío, Tello Ayala y Parkes condiciona cada test en
el resultado positivo o negativo de los anteriores, y cierra señalando como
...
infectados en el pool. La motivación no es artificial. La qPCR devuelve un entero
(el cycle threshold) que es, a primer orden, monótono en la carga viral del pool,
...
La población tiene N individuos, cada uno con utilidad u_i y prior de infección
p_i, infecciones independientes, presupuesto de B tests y pools de a lo más G. Se
...
muestra la deducción cruzada: de r_2 = 0 se sabe que 1 y 2 están sanos, y con
r_1 = 1 se concluye que 0 está infectado, sin haberlo testeado solo.
...
de cardinalidad. La Proposición 1 prueba que calcular la marginal exacta
...
dentro de un pool pero conservan el conteo total de infectados, así que la cadena
queda atrapada en el nivel de conteo donde arrancó: en {0,1}=1, {1,2}=1 con prior
0.15 la posterior exacta es (0.15, 0.85, 0.15), pero un sampler de solo swaps
...
movimiento que a los samplers locales les falta.
...
es la cosecha de sanos deducidos: la utilidad se acredita solo al colocar a un
individuo en un pool con r = 0, así que un individuo deducido sano conserva su u_i
...
qPCR, donde el conteo se observa solo a través de un cycle threshold ruidoso.
