...
\item[\textbf{2.}] \textbf{Simulation with overhead.} \lab{DEMOSTRADO --- revisión cruzada de B pendiente; cota VERIFICADA $n\le5$} Let $D(h)=\Cpz(h)\setminus\Cstr(h)$ be the set accredited by deduction only, and for a policy $\pi$ let $\Delta(\pi)$ be the maximum over its trajectories of the number of deduced-clean pieces at the end. Every posterior-zero policy with budget $B$ is replicated under strict clearing with $\Delta(\pi)\le B-1$ additional tests, one per piece, so
\[
\OPT_\pz(B)\;\le\;\OPT_\str\big(B+\Delta(\pi^*)\big)\;\le\;\OPT_\str(2B-1).
\]
Witness in the four-person game of Part 3(b): $\OPT_\pz(2)=0.774\le\OPT_\str(3)=1.011$; the bound $\OPT_\pz(B)\le\OPT_\str(2B-1)$ holds on every instance of the verification grid ($n\le5$, $B\le3$, homogeneous and heterogeneous), with equality at $B=1$.
...
\item[(c)] \emph{Scale.} \lab{cota realizable} At the anchor $(q,G,B)=(0.05,16,7)$, cover-then-bisect constructions guarantee at least $1-0.95^{48}\approx0.9147$ under posterior-zero and $1-0.95^{32}\approx0.8063$ under strict. Reserving the additional strict certification test reduces the number of root pools from three to two, losing one pool's worth of coverage. These are achievable lower bounds, not the full expected utilities of the constructions (Remark~\ref{rem:anchor}).
\end{enumerate}
\end{enumerate}
\end{proposition}
...
Throughout, $0<p_i<1$ for every $i$, so a profile is consistent with $h$ exactly when it has positive posterior weight.

\emph{(i) What deduction accredits.} In a laminar history the tested people are partitioned into residual cells, and the count of every cell is determined by $h$. If a cell $X$ has count $r$ with $0<r<|X|$, every arrangement of $r$ infected among its members is consistent, so every member has positive infection posterior; if $r=|X|$ nobody in it is healthy. Hence $\Cpz(h)$ is the union of the cells with count $0$, and $D(h)$ is the union of the \emph{deduced-clean pieces}: cells with count $0$ that were never themselves a tested pool reading $0$.

\emph{(ii) Birth and size.} A fresh pool is disjoint from everything tested and its count is observed directly: if it reads $0$ its members hold a strict certificate, otherwise it becomes an atom with an interior count. Only a refinement creates a deduced-clean piece: refining the atom $X$ (count $r$) by $S\subsetneq X$ (count $s$) leaves $X\setminus S$ with count $r-s$, deduced-clean exactly when $s=r$. Since $|X|\le G$ and $|S|\ge1$, the piece has at most $G-1$ people, so a single strict test on the whole piece certifies it; pieces from different atoms cannot share a test, because a legal pool is fresh or contained in one atom. A refinement creates at most one such piece, and a posterior-zero policy never refines a deduced-clean piece, whose members it has already accredited.

\emph{(iii) The copycat.} Let $\pi$ be a posterior-zero policy with budget $B$, and run it under the strict rule. Legality depends only on $h$: every fresh pool and every refinement of an interior atom is legal under strict as well, whose menu merely adds tests on deduced-clean pieces. The outcome distribution of a test depends only on the priors and on $h$, not on the accreditation rule. So the copied run produces the same histories with the same probabilities.

\emph{(iv) Certification.} When $\pi$ stops, run one test on each deduced-clean piece of $h_{\rm final}$. Each is legal (a pool inside a live atom of count $0$, of size at most $G$), reads $0$ with certainty (the history already implies it), and accredits the piece. Every posterior-zero accredited person now holds a strict certificate: those who sat in a zero-count pool already did, and the deduced ones just received one. Hence $V_\str(\pi')\ge V_\pz(\pi)$ for the extended policy $\pi'$ (in fact equality, by Part 1 and because the certification tests are uninformative). On the trajectory ``pair reads 1, then $\{1\}$ reads 1'' of Part 3(b) the extra test is $\{2\}$.

\emph{(v) The bill.} The first test is never a refinement, so a trajectory of $B$ tests contains at most $B-1$ refinements and therefore at most $B-1$ deduced-clean pieces: $\Delta(\pi)\le B-1$. The budget is hard on every path, so $\pi'$ needs $B+\Delta(\pi)$ tests, the maximum over trajectories and not the mean. Taking $\pi$ optimal, $\OPT_\pz(B)\le V_\str(\pi')\le\OPT_\str(B+\Delta(\pi))\le\OPT_\str(2B-1)$.
\end{proof}
...
\begin{remark}[Anchor bounds and an explicit suboptimal policy]\label{rem:anchor}
Map 3 (B, 2026-09-21, \texttt{results/mapa\_homogeneo\_3.csv}): at the frontier $n=24$, $G=8$, $B=6$ the exact laminar optimum is $0.8654$ against the cover-then-bisect bound $0.7080$. The explicitly evaluated variant covers until the first root with a healthy member, bisects it, prefers the left child when both contain healthy members, and stops at the first certification; its expected utility is $0.7271$. In the anchor population, a feasible policy restricted to $G\le8$ achieves $1.1233$, exceeding both the bound $0.9147$ and the same explicit CBS variant's value $0.9394$. Thus that variant is suboptimal and the anchor bound is slack. Other continuations of CBS require their own definition and evaluation; the full anchor $G=16$ is not certified (state limit). Part 3(c) states achievable lower bounds under the two conventions, not their exact policy values or optima.
\end{remark}

\section{Verification by enumeration}

The Bellman solver on residual atoms (\texttt{augmented/bm17\_toy\_solver.py}) is checked against explicit-profile enumeration over histories (\texttt{augmented/enumerador\_historias.py}) and independently against B's \texttt{augmented/pathwise\_enum.py}. In \texttt{tests\_brecha\_convencion.py}, optima and all forced first actions agree exactly in rational arithmetic on the declared homogeneous and heterogeneous cases with $n\le5$, $B\le3$, under both conventions. The BM17 suite also includes homogeneous roots up to $n=6$. Together, \texttt{tests\_brecha\_convencion.py} and \texttt{tests\_bm17.py} pass 626 tests (2026-09-22). The table gives $n=4$, $G=2$, $B=2$, $u\equiv1$, where the formulas of Part 3(b) are exact:
\begin{center}
