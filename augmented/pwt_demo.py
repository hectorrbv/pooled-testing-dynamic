"""
PWT (per Francisco Marmolejo) — formulation check, counterexample, and
empirical comparison on n=6, G=3, B=2.

Setup. After k tests, the covered set S = ∪ pools is built. V = N \\ S.
Francisco proposes augmenting the next-step problem with super-nodes
...
Empirical claims demonstrated here:
  1. With prob_A and util_T = Σ u_i, scalar PWT EXACTLY reproduces the
     standard myopic-greedy step value (utility-of-pool decomposes
     additively, all-clear probability decomposes multiplicatively).
  2. With prob_B, scalar PWT solves a DIFFERENT objective and can pick
     worse pools.
...
def _enumerate_pwt(S_idx, V_idx, p, u, G, prob_mode):
...
def main():
    ...
    print(f"  full pool enum.   : pool={indices_from_mask(pool_full)}  "
          f"val={val_full:.6f}")
    print(f"  PWT  (all-clear)  : pool={indices_from_mask(pool_A)}  "
          f"val={val_A:.6f}")
    print(f"  PWT  (or-event)   : pool={indices_from_mask(pool_B)}  "
          f"val={val_B:.6f}")
    print(f"  val(PWT-A) == val(full)? {abs(val_A - val_full) < 1e-9}")
    print(f"  val(PWT-B) == val(full)? {abs(val_B - val_full) < 1e-9}")
    ...
    print("  PWT scalar formulation reproduces the myopic objective exactly")
    ...
    print("  PWT scalar formulation is myopic by construction → it lives on")
    print("  the 'greedy(true)' line. To close the gap, PWT must carry the")
