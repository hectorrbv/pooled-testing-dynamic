"""
Solver-based optimal group selection for large-n instances.

Two backends:
  mosek_best_group()  — exponential cone (exact via Mosek Fusion API)
  gurobi_best_group() — MILP with log general constraint (Gurobi)

Both return a group bitmask, same interface as greedy._myopic_best_group().
...
def _heuristic_best_group(active_indices, p, u, G):
...
def mosek_best_group(p, u, G, n, cleared_mask):
    """Find optimal myopic group using Mosek exponential cone.
    ...
    Returns group bitmask (0 if no useful group).
    """
    ...
    # Check: all utilities zero → no useful group
    ...
    try:
        ...
        with Model('mosek_group') as M:
            ...
            M.constraint("group_max", Expr.sum(x),
                          Domain.lessThan(float(G)))
            M.constraint("group_min", Expr.sum(x),
            ...
            if sol_status not in (SolutionStatus.Optimal,
                ...
                return _heuristic_best_group(active_indices, p, u, G)
        ...
        warnings.warn(
            f"mosek_best_group failed ({e}), using heuristic fallback",
            RuntimeWarning, stacklevel=2,
        )
        return _heuristic_best_group(active_indices, p, u, G)


def gurobi_best_group(p, u, G, n, cleared_mask):
    """Find optimal myopic group using Gurobi MILP with log constraint.
    ...
    try:
        ...
        with gp.Env(empty=True) as env:
            ...
            with gp.Model('gurobi_group', env=env) as m:
        ...
        warnings.warn(
            f"gurobi_best_group failed ({e}), using heuristic fallback",
            RuntimeWarning, stacklevel=2,
        )
        return _heuristic_best_group(active_indices, p, u, G)


def solver_best_group(p, u, G, n, cleared_mask, solver='mosek'):
    ...
    Returns group bitmask.
    """
    if solver == 'mosek':
        return mosek_best_group(p, u, G, n, cleared_mask)
    elif solver == 'gurobi':
        return gurobi_best_group(p, u, G, n, cleared_mask)
    else:
