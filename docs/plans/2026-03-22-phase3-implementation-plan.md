...
1. **Gibbs Verification** — Prove Gibbs drawing is correct (CRITICAL)
2. **Beta Meta-parameter** — LatentState discovery reward for greedy
...
| State-aware scoring (alpha) | `hybrid_solver.py:latent_state_aware_score` | Done |
...
| Gibbs drawing | `bayesian.py:gibbs_update` | Done (9 basic tests) |

---

## SPRINT 1: Gibbs Drawing Verification (CRITICAL — DO FIRST)
...
## SPRINT 2: LatentState Discovery Reward (Beta Meta-parameter)

### 2.1 Core Concept

The existing `latent_state_aware_score` in hybrid_solver.py blends myopic clearing gain with information gain using alpha. But Francisco wants a **separate, simpler mechanism** for the standard greedy:

> "Agregar un metaparametro a greedy en forma de premio para encontrar a personas activas, puedes agregale un pseudopremio. Es en esencia darle una manera indirecta de pensar a futuro."

### 2.2 How it differs from existing mechanisms

| Mechanism | File | What it does |
|-----------|------|-------------|
| Semi-utility (alpha) | `semi_utility.py` | Values partial clearance confidence |
| State-aware (alpha) | `hybrid_solver.py` | Blends myopic + info gain in hybrid solver |
| **Beta reward (NEW)** | **`state_reward_greedy.py`** | **Pseudo-reward for confirming latent_states in standard greedy** |

Beta is specifically about: "If I test this pool and find r > 0, I learn WHO is active and can stop wasting tests on them."

### 2.3 Implementation
**File:** New `augmented/state_reward_greedy.py`
...
```python
# VIP: 8 professors, high latent_state, very high utility
p_vip = [0.8] * 8;    u_vip = [10.0] * 8
# Regular: 12 others, moderate latent_state, lower utility
...
2. **Does beta help?** → VIP experiment: latent_state reward greedy vs standard
