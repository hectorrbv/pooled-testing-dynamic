...
**Architecture:** Create `augmented/state_reward_greedy.py` as a self-contained companion to `augmented/greedy.py`. Keep the core beta-greedy policy exact over test outcomes, and restrict randomized scenario generation to the reporting helpers `run_vip_benchmark()` and `run_beta_sweep()`.
...
**Files:**
- Create: `augmented/tests_state_reward_greedy.py`
...
Run: `python augmented/tests_state_reward_greedy.py`
Expected: FAIL because `augmented.state_reward_greedy` does not exist yet.

### Task 2: Implement the beta-reward greedy module

**Files:**
- Create: `augmented/state_reward_greedy.py`
...
**Files:**
- Verify: `augmented/state_reward_greedy.py`

**Step 1: Run the module**

Run: `python augmented/state_reward_greedy.py`
