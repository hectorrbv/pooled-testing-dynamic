# Estado del proyecto — Dynamic Augmented Adaptive Group Counting (lenguaje sencillo)
...
- **posterior:** la probabilidad **actualizada** de que alguien esté activo,
  después de ver resultados de tests.
- **posterior conjunto exacto:** la probabilidad real de que **un grupo** salga
  conteo-no-cero. Ojo: tras varios tests, las personas quedan **relacionadas**, así que
...
- **PMF (del conteo):** la lista de probabilidades de cada posible **número** de
  conteo-no-ceros en un grupo (0, 1, 2, …).
...
Tienes `n` personas, cada una con una probabilidad de estar activa `p_i` y un
valor `u_i` por declararla limpia. Con pocos tests `B` (cada uno mezcla hasta `G`
personas) quieres **maximizar el valor de la gente que logras probar limpia**. La
versión "augmented" supone que el test te dice el **número exacto** de activos
...
| Variante con peso β / ganancia de info | `state_reward_greedy.py` | ✅ |
...
| **Estudio preliminar de prevalencia** (augmented vs clásico al subir estado latente) | `experiments.py` (barre rangos low/med/high) | ✅ preliminar |
...
5. **Robustez a alta prevalencia.** Hay un estudio **preliminar**
   ...
   final (la hipótesis: en augmented "no se rompe tanto" al subir la estado latente).

6. **Direcciones grandes del paper.** **Fairness** e **incentivos**: sin empezar.
   **Falsos conteo-no-ceros/conteo-ceros** y **4 utilidades por persona**: sin empezar
   (lo único parecido es `semi_utility.py`).

7. **RL a escala.** Falta **entrenar de verdad** el PPO bucket (N=50, ~200k pasos)
   y reportar "RL le gana al greedy". Lo de hoy fueron consultas cortas.
...
- **Resultados PPO aún no "oficiales":** ya hay una ruta para comparar PPO vs DP y
  en una consulta corta (CSV N=3) dio igual al óptimo, pero **falta guardarlo como
