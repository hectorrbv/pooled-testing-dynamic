...
1. **Finster, González Amador, Lock, Marmolejo-Cossío, Micha, Procaccia —
   "Welfare-Maximizing Adaptive Group Counting"** (arXiv:2206.10660, EC'23). El paper
...
2. **Lopez, Marmolejo-Cossío, Tello Ayala, Parkes — "Dynamic Welfare-Maximizing
   Adaptive Group Counting"** (paper). La **extensión dinámica** del anterior.
...
En Finster/tesis: `u_i` = utilidad por quedar en un test **conteo-cero**; `p_i` =
probabilidad de estar **SANO**. Bienestar de un plan T: `u(T) = Σ u_i · P_i^T`,
con `P_i^T` = prob. de quedar en ≥1 test conteo-cero. Un pool es conteo-cero sii todos
limpios. El **proyecto augmented invierte la convención** (`p_i` = prob. activo)
y además usa el **conteo exacto** `r=|t∩Z|` en vez de conteo-no-cero/conteo-cero binario;
...
| Non-grouped estático | `O(N log B)` | tesis A.3 |
...
- **NO hay** submodularidad ni garantía `(1−1/e)` en ninguno de los tres
  documentos, y **no se consulta NP-hardness** (solo se menciona el CSP de
...
- **Greedy dinámico usa solo marginales, no la conjunta** → puede calcular mal la
  prob. de un test conteo-cero y elegir pools subóptimos (tesis Ejemplos 4.3, 4.4;
...
- **Independencia** de estados de estado latente (justificada por lockdown); no modela
  correlaciones.
...
- **Secuencialidad:** el dinámico exige esperar resultados antes del siguiente
  pool (ventana de validez de registros: VIH ~24h, Hep. C hasta 168h).
...
- Tabla 6.1 (bienestar promedio, N=G=5,B=3): Non-Grouped 1.10, Greedy Non-Ov 1.03,
...
- `p_i` = activo (no limpio); el `bayesian_update_by_counting` del repo enumera
