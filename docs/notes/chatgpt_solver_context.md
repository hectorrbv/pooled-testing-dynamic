# Context for Understanding `solver.py` — Dynamic Augmented Adaptive Group Counting

You are helping a graduate researcher understand their implementation of a brute-force DP solver for Dynamic Augmented Adaptive Group Counting Strategies (DAPTS). Below you will find:
...
- Each individual has utility **u_i ≥ 0** and latent-state probability **p_i ∈ [0,1]**, with **q_i = 1 − p_i**.
- Population instance: **J = (p_1,...,p_n, u_1,...,u_n)**.
- Budget of **B ≥ 1** augmented grouped tests.
- Maximum pool size **G** (physical limit): each test t ⊆ [n] must satisfy |t| ≤ G.
- **P_G(S)** = {U ⊆ S : |U| ≤ G} — all subsets of S with cardinality at most G.

### LatentState Profile
Each individual is active independently: Z_i ~ Bernoulli(p_i). The latent-state profile is **Z = (Z_1,...,Z_n) ∈ {0,1}^n**. The probability of a specific profile Z is:

$$\Pr(Z) = \prod_{i=1}^{n} p_i^{Z_i} \cdot q_i^{1-Z_i}$$

### Augmented Test Result (Idealized)
For a pool t ⊆ [n] and latent-state profile Z, the **augmented test result** is:

$$r(t, Z) = \sum_{i \in t} Z_i = |t \cap \{i : Z_i = 1\}|$$

This returns the **exact count** of active individuals in the pool (not just binary +/−).

### Definition 1 — Counting History
A counting history of length k is a list of k test-result pairs:
...
### Definition 2 — DAPTS
A Dynamic Augmented Adaptive Group Counting Strategy is **F = (F_1, ..., F_B)** where:
...
Given F and a specific latent-state profile Z, the tests unfold recursively:
...
An individual earns utility only if they appeared in some pool that tested **completely zero-count** (r = 0), which proves they are clearancey.
...
where:
- **U^single**: optimal individual counting (test one person per test, pick top B by u_i·q_i)
...
def test_result(pool_mask: int, z_mask: int) -> int:
    ...
    This is the number of active individuals in the pool.
    """
...
- `z_mask` = latent-state profile Z ∈ {0,1}^n, represented as bitmask
...
### `simulator.py` — Executes a DAPTS on a Fixed LatentState Profile
...
def apply_dapts(F, z_mask, n, u):
    ...
    for k in range(1, F.B + 1):
        ...
        if r == 0:
            cleared_mask |= pool   # all individuals in pool proven clearancey
...
State = (step k, remaining_set, cleared_mask)
  ...
  - cleared_mask: bitmask of individuals proven clearancey
...
def solve_optimal_dapts(
    ...
    ----------
    p : list[float]
        Latent-state probabilities, length n.
    u : list[float]
        Utilities, length n.
    B : int
    ...
    # Precompute weight w[z] = Pr(Z = z) for every latent-state profile
    num_profiles = 1 << n
    ...
    def dp(k: int, remaining: frozenset, cleared_mask: int) -> Tuple[float, int]:
        ...
        cleared_mask = individuals proven clearancey so far.
        """
...
1. **Maps every variable to the paper's math**: For example, `remaining` ↔ the set of latent-state profiles Z consistent with observations so far; `cleared_mask` ↔ the set of individuals proven clearancey; `w[z]` ↔ Pr(Z = z); etc.
