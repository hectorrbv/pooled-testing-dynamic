...
`augmented/bayesian.py :: gibbs_update` podía **contar perfiles de estado latente
...
1. **Init inválido.** El init greedy (asignaba activos para cumplir cada
...
4. **Sin guard al recolectar.** La registro se contaba **sin** checar validez.

### Evidencia (escenarios que sí usan el MCMC, >7 activos)

- Fracción de **registros inválidas** contadas: media **~30%**, máx **100%**.
...
- Invariante duro `Σ marginales sobre un pool = r` (debe ser exacto porque todo
  perfil válido tiene `r` activos ahí): el exacto lo cumple a 1e-15; el Gibbs
  lo violaba hasta **2.0–3.0** → consulta matemática de que contaba inválidos.

Scripts: `augmented/notebooks/gibbs_validity_audit.py` (función real) y
`gibbs_validity_audit2.py` (copia instrumentada: cuenta registros inválidas y
...
2. **Init válido** por búsqueda de mínimos conflictos (`_find_valid_state`) para
   la rama MCMC.
3. **Guard de validez**: solo se cuentan registros válidas; `total_draws` y la
...
4. **Fallback exacto capado** (`≤ EXACT_ACTIVE_FALLBACK_CAP = 22`) si el MCMC no
   junta ninguna registro válida — evita cuelgues (refinamiento de Codex).
...
activos** — no pueden conectar perfiles válidos con distinto total (ej.:
