...
4. Luego aclaramos cómo entra la utilidad: solo ganamos cuando un pool sale completamente conteo-cero.
...
El greedy empieza testeando una sola persona, `{1}`. Esa decisión tiene sentido si uno piensa solo en el beneficio inmediato. Parece una jugada limpia: testeo a alguien, si sale limpio lo libero, y sigo.

El óptimo hace algo menos obvio: empieza con `{1,2}`. Ese pool puede devolver 0, 1 o 2 activos. Cada resultado abre una rama distinta para el segundo test. Entonces el primer test no se valora solo por lo que libera hoy, sino por la información que deja para decidir mañana.
...
Esta gráfica es importante porque registro, con el ejemplo más simple posible, que dos tests juntos pueden decir algo que cada test por separado no deja ver.
...
El segundo test dice que en `{1,2}` hay cero activos. Entonces 1 y 2 están limpios.

Pero el primer test decía que en `{0,1}` había exactamente un activo. Como ya sabemos que 1 está limpio, el activo tiene que ser 0.

Por eso la actualización correcta da:

```text
P(0 activo | historia completa) = 1.00
```
...
El punto no es que hacer Bayes secuencial sea malo. El punto es que si en cada paso solo guardamos marginales individuales, podemos perder relaciones importantes. Aquí la relación es muy clara: un test vuelve limpios a 1 y 2, y eso obliga al otro test a señalar a 0.
...
Esta gráfica es interesante porque registro qué pasa después de observar un conteo exacto dentro de un pool pequeño.
...
```text
exactamente uno de los dos está activo
```

Antes del test, podíamos pensar en 3 y 6 como dos personas separadas. Después del test, ya no. Si 3 está activo, 6 tiene que estar limpio. Si 6 está activo, 3 tiene que estar limpio.
...
No quedan 0.5 y 0.5 porque antes del test 3 ya era más riesgoso que 6. Condicionado a que exactamente uno está activo, es más probable que sea 3.

Pero lo más importante es esto: no podemos tomar `q3 = 0.642` y `q6 = 0.358` como si fueran independientes. La probabilidad de que ambos estén activos no es `0.642 * 0.358`. Es cero, porque el conteo dijo que exactamente uno está activo.

La frase clave sería:

El super-nodo aparece porque el conteo no solo cambia probabilidades; crea una dependencia.

Esta gráfica sirve para explicar por qué augmented adaptive group counting necesita guardar más estructura que solo probabilidades individuales.
...
## Gráfica 4: Utilidad y pools completamente conteo-ceros

Esta gráfica aclara algo básico del objetivo: la utilidad solo se gana cuando el pool sale completamente conteo-cero.

Si un pool tiene conteo 0, todos sus miembros están limpios y se pueden liberar. Ahí se gana la suma de sus utilidades.

La probabilidad de que eso pase es:

```text
P(todo limpio) = producto de q_i
```

donde `q_i` es la probabilidad de que la persona `i` esté limpia.

Entonces el valor aproximado del pool es:

```text
valor = P(todo limpio) * suma de utilidades
```

La gráfica registro el tradeoff. Meter más personas puede subir la utilidad potencial, pero también baja la probabilidad de que todo el pool salga limpio.

El ejemplo más claro es la persona E. E tiene utilidad alta, pero también es riesgosa. Al meterla, la probabilidad de que todo el pool sea limpio cae mucho.

Números de la gráfica:

```text
pool {A,B,C,D}:   P(todo limpio) = 0.252, valor = 2.52
pool {A,B,C,D,E}: P(todo limpio) = 0.076, valor = 1.06
mejor pool {A,B,C}: P(todo limpio) = 0.504, valor = 3.53
```
...
El algoritmo no busca pools grandes; busca pools con buen balance entre utilidad y probabilidad de salir conteo-ceros.

Esta gráfica ayuda a separar dos ideas: utilidad potencial y probabilidad de cobrar esa utilidad.

Preguntas para Francisco:

1. ¿Qué pesa más en la decisión de un pool: sumar utilidad alta o proteger la probabilidad de que salga completamente conteo-cero?
2. ¿Esta forma de valorar pools captura bien el objetivo real, o también deberíamos valorar información aunque el pool no salga conteo-cero?
...
```text
{0,1,2} tiene 1 activo
{2,3,4} tiene 1 activo
```
...
```text
{2} tiene 1 activo total
los otros cuatro tienen 2 activos totales
```

El movimiento tipo Gibbs que se ilustra intercambia un limpio por un activo. Ese movimiento mantiene fijo el total de activos. Entonces puede moverse dentro del grupo rojo, pero no puede cruzar hacia el perfil azul `{2}`.

El problema es sutil: el generator puede estar haciendo movimientos válidos, pero aun así no recorrer todo el espacio.

La frase clave sería:

No basta con generar perfiles válidos; el método también tiene que poder llegar a todos los perfiles válidos.

Esta gráfica justifica tener cuidado con Gibbs o con cualquier generator local. Si la fibra se parte, una cadena puede quedarse encerrada en una componente y estimar mal las probabilidades.

Preguntas para Francisco:

1. ¿Qué generator se quedaría atrapado en una sola componente de esta fibra?
